...
class SemiSupLabeler():
    ...
    def __init__(self, data_lab, data_unlab, data_submit):
      ...
      if (self.JSON_MODE):
          ...
          if os.path.isfile(fn):
              ...
              if (self.EARLY_STOP_MODE): self.patience = self.json_dict['patience']
          ...
          if (self.EARLY_STOP_MODE): self.jsondict['patience'] = self.patience
    ...
    def label_spr(self):
      ...
      for i in range (self.manyfit):
        ...
        if (self.ss_mod == 'LabSpr' and self.ss_kern == 'knn'): 
          self.label_prop_model = LabelSpreading(kernel='knn', gamma = self.gamma, 
                                                 n_neighbors=self.neighbors, alpha = self.alpha)
        
        elif (self.ss_mod == 'LabProp' and self.ss_kern == 'rbf'):
                self.label_prop_model = LabelPropagation(kernel = 'rbf', gamma = self.gamma, 
                                                         n_neighbors = self.neighbors, 
                                                         alpha = self.alpha, max_iter = 10)
        else:            
    ...
    def init_variables(self):
      ...
      X_train_lab, X_valid_lab, self.y_train, self.y_valid = train_test_split(X_big_lab, y_big, 
                                                                              test_size = (1 - self.RATIO), 
                                                                              random_state = 14)
    ...
    def nn_fit(self,X,y):
      ...
      if (self.EARLY_STOP_MODE):
          call_back_list.append(EarlyStopping( patience = self.patience, 
                                                verbose = 1, mode ='min',
