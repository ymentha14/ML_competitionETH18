...
def __init__(self,data_lab,data_unlab,data_submit):
  ...
  self.param_list = ['Ratio','pca','UsingNN','paramsout','data_state','scaler']+params_nn+params_ss
  ...
  if (self.JSON_MODE):
      ...
      if os.path.isfile(fn):
          ...
          selfcheck(self.json_dict,self.param_list)
          self.check(self.json_dict['paramsout'],self.param_list)
...
def init_variables(self):
  ...
  if (self.self.scaler == 'Standard'):
     scaler = StandardScaler()
  elif (self.self.scaler == 'Normal'):
     scaler = Normalizer()
  else:
     scaler = StandardScaler()
...
def fit_lab(self):
  self.nn_fit(self.X_train_lab)

def fit_tot(self):
  self.nn_fit(self.X_tot)
...
def complete_unlab(self):
  y_missing = model.predict(self.X_unlab)
